...
### <a name="anchors_get"></a>get

Gets anchor information for the directory block containing the given object identifier.

**Sample**
```python
factom_client.anchors.get('4b9532c79d53ab22b85951b4a5853f81c2682132b3c810a95128c30401cd1e58')
```
...
|------------------------------|----------|------------------------------------------------------------------------------|---------------------------------------------------------------------|
| `object_identifier`                   | required | string </br> Represented by an entry hash, chain id, directory block height, directory block keymr, entry block keymr, or factoid block keymr.                                                                                                                                                                                                                            | **object_identifier must be a positive int or a string of length 64** |
...
-   **data:** object </br> Contains information about the desired directory block's anchors
	-   **data.dblock:** object </br> The directory block that was anchored
  ...
  -   **data.anchors:** [object] </br> Contains detailed information about the returned anchors
      ...
      -   **data.anchors.status** string </br> Describes the current state of the anchoring process ("pending" or "confirmed")
      -   **data.anchors.tx_id** string </br> The transaction hash for the anchor (only present if anchor status is "confirmed")
      -   **data.anchors.block_hash** string </br> The hash of the block that includes the anchor transaction (only present if anchor status is "confirmed")
...
-   **data:** object </br> Contains information about the desired directory block's anchors
  ...
  -   **data.anchors:** [object] </br> Contains detailed information about the returned anchors
      -   **data.anchors.network** string </br> The network being anchored to
      -   **data.anchors.status** string </br> Describes the current state of the anchoring process ("pending" or "confirmed")
      -   **data.anchors.created_at** string </br> The timestamp for this entry, sent in ISO 8601 format (i.e. YYYY-MM-DDThh:mm:ssZ)
      -   **data.anchors.entry_serialized** string </br> The raw data that makes up the anchor entry
      -   **data.anchors.entry_hash** string </br> The unique identifier of the entry
      -   **data.anchors.merkle_branch** [object] </br> The branch of the merkle tree that represents the anchor entry's path from entry hash to directory block key Merkle root, presented as an array of Merkle nodes
      ...
      -   **data.anchors.dblock** string </br> The mainnet directory block that contains the anchor entry
          -   **data.anchors.dblock.keymr** string </br> The Key Merkle Root for this directory block
          -   **data.anchors.dblock.height** string </br> The sequence number of this directory block
