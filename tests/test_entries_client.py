...
class TestEntriesClientWithoutSigning(TestCase):
    ...
    def test_get(self):
        ...
        with patch("factom_sdk.request_handler.request_handler.requests.request") as mock_get:
            ...
            response = self.entries_client.get("123", "123", signature_validation=False)
        ...
        with patch("factom_sdk.request_handler.request_handler.requests.request") as mock_get:
            ...
            response = self.entries_client.get("123", "123", signature_validation=signature_validation)
            self.assertIsNotNone(response)
    ...
    def test_list(self):
        ...
        with self.assertRaises(Exception) as cm:
            self.entries_client.list("123", limit="1")
        self.assertTrue("limit must be an integer." in str(cm.exception))

        with self.assertRaises(Exception) as cm:
            self.entries_client.list("123", limit=1, offset="1")
        self.assertTrue("offset must be an integer." in str(cm.exception))

        with self.assertRaises(Exception) as cm:
            self.entries_client.list("123", limit=1, offset=1, stages="1")
        self.assertTrue("stages must be an array." in str(cm.exception))

        with patch("factom_sdk.request_handler.request_handler.requests.request") as mock_get:
            mock_get.return_value.ok = True
            response = self.entries_client.list("123", limit=1, offset=1, stages=["1"])
        self.assertIsNotNone(response)
    ...
    def test_get_first(self):
        ...
        with patch("factom_sdk.request_handler.request_handler.requests.request") as mock_get:
            mock_get.return_value.ok = True
            response = self.entries_client.get_first("123", signature_validation=False)
        self.assertIsNotNone(response)
        ...
        with patch("factom_sdk.request_handler.request_handler.requests.request") as mock_get:
            ...
            response = self.entries_client.get_first("123", signature_validation=signature_validation)
        self.assertIsNotNone(response)
    ...
    def test_get_last(self):
        ...
        with patch("factom_sdk.request_handler.request_handler.requests.request") as mock_get:
            mock_get.return_value.ok = True
            response = self.entries_client.get_last("123", signature_validation=False)
        self.assertIsNotNone(response)
        ...
        with patch("factom_sdk.request_handler.request_handler.requests.request") as mock_get:
            ...
            response = self.entries_client.get_last("123", signature_validation=signature_validation)
        self.assertIsNotNone(response)
    ...
    def test_search(self):
        ...
        with self.assertRaises(Exception) as cm:
            self.entries_client.search("123", ["1"], limit="1")
        self.assertTrue("limit must be an integer." in str(cm.exception))

        with self.assertRaises(Exception) as cm:
            self.entries_client.search("123", ["1"], limit=1, offset="1")
        self.assertTrue("offset must be an integer." in str(cm.exception))

        with patch("factom_sdk.request_handler.request_handler.requests.request") as mock_post:
            mock_post.return_value.ok = True
            response = self.entries_client.search("123", ["1"], limit=1, offset=1)
        self.assertIsNotNone(response)
...
class TestEntriesClientWithSigning(TestCase):
    ...
    def test_create(self):
        ...
        with self.assertRaises(Exception) as cm:
            self.entries_client.create("123", "123", external_ids=["123"],
                                       signer_private_key="idsec1Xbja4exmHFNgVSsk7VipNi4mwt6BjQFEZFCohs4Y7TzfhHoy6",
                                       signer_chain_id="123", callback_url="google")
        self.assertTrue("callback_url is an invalid url format." in str(cm.exception))

        with self.assertRaises(Exception) as cm:
            self.entries_client.create("123", "123", external_ids=["123"],
                                       signer_private_key="idsec1Xbja4exmHFNgVSsk7VipNi4mwt6BjQFEZFCohs4Y7TzfhHoy6",
                                       signer_chain_id="123", callback_stages="123")
        self.assertTrue("callback_stages must be an array." in str(cm.exception))

        with patch("factom_sdk.request_handler.request_handler.requests.request") as mock_post:
            mock_post.return_value.ok = True
            response = self.entries_client.create("123", "123", external_ids=["123"],
                                                  signer_private_key="idsec1Xbja4exmHFNgVSsk7VipNi4mwt6BjQFEZFCohs4Y7TzfhHoy6",
                                                  signer_chain_id="123", callback_url="http://google.com", callback_stages=["123"])
        self.assertIsNotNone(response)
